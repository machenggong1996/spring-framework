...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	protected <T> T doGetBean(String name, @Nullable Class<T> requiredType, @Nullable Object[] args,
		...
		// 2.尝试从缓存中获取beanName对应的实例 允许提前曝光解决循环依赖 getSingleton(beanName, true)
			...
			if (System.getSecurityManager() != null) {
				return AccessController.doPrivileged((PrivilegedAction<Boolean>) () -> (
						(fb instanceof SmartFactoryBean && ((SmartFactoryBean<?>) fb).isPrototype()) || !fb
								.isSingleton()), getAccessControlContext());
			} else {
	...
	public void addBeanPostProcessor(BeanPostProcessor beanPostProcessor) {
		...
		if (beanPostProcessor instanceof DestructionAwareBeanPostProcessor) {
			// 4.如果beanPostProcessor是DestructionAwareBeanPostProcessor,
			// 则将hasInstantiationAwareBeanPostProcessors设置为true,
	...
	protected void registerCustomEditors(PropertyEditorRegistry registry) {
		...
		if (!this.customEditors.isEmpty()) {
			this.customEditors.forEach((requiredType, editorClass) -> registry
					.registerCustomEditor(requiredType, BeanUtils.instantiateClass(editorClass)));
		}
	}
	 ...
	 * @return a (potentially merged) RootBeanDefinition for the given bean
	 * @throws BeanDefinitionStoreException in case of an invalid bean definition
	 * MergedBeanDefinition：这个词其实不是一个官方词，但是很接近，该词主要是用来表示 “合并的 bean 定义”，因为每次都写 “合并的 bean 定义” 有点太绕口，因此我在之后的注释或解析中或统一使用
	 * MergedBeanDefinition 来表示 “合并的 bean 定义”。
	 *
	 ...
	 * 该 BeanDefinition 不存在 “父定义”，但是该 BeanDefinition 的类型不是 RootBeanDefinition：使用该 BeanDefinition 的参数构建一个
	 * RootBeanDefinition。
	 ...
	 * RootBeanDefinition。
	 *
	 * 在我们日常使用的过程中，通常会是上面的第3种情况。如果我们使用 XML 配置来注册 bean，则该 bean 定义会被封装成：GenericBeanDefinition；如果我们使用注解的方式来注册
	 * bean，也就是<context:component-scan /> + @Compoment，则该 bean 定义会被封装成 ScannedGenericBeanDefinition。
	 */
	protected RootBeanDefinition getMergedBeanDefinition(String beanName, BeanDefinition bd,
		...
		try {
			...
			if (System.getSecurityManager() != null) {
				return AccessController
						.doPrivileged((PrivilegedExceptionAction<Class<?>>) () -> doResolveBeanClass(mbd,
						                                                                             typesToMatch),
	...
	protected boolean requiresDestruction(Object bean, RootBeanDefinition mbd) {
		return (bean.getClass() != NullBean.class && (DisposableBeanAdapter.hasDestroyMethod(bean, mbd) || (
				hasDestructionAwareBeanPostProcessors() && DisposableBeanAdapter
						.hasApplicableProcessors(bean, getBeanPostProcessors()))));
	}
